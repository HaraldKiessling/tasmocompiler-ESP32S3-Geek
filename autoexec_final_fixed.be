...
class SensorDashboard : Driver
    ...
    def every_second()
        ...
        for i : 0 .. 2
            if bme_labels[i] != nil
                if i < size(bme_sensors)
                    ...
                    if hum > 0
                        bme_labels[i].set_text(string.format("..%s %.1f°C %.0f%%", short_id, temp, hum))
                    else
                        bme_labels[i].set_text(string.format("..%s %.1f°C", short_id, temp))
                    end
                else
                    bme_labels[i].set_text("")
                end
            end
        end
        
        # --- DS18x20 Sensoren zweispaltig (p1b20-p1b27) ---
        var ds_sensors = []
        ...
        var ds_labels = [
            global.p1b20, global.p1b21,  # Zeile 1
            global.p1b22, global.p1b23,  # Zeile 2
            global.p1b24, global.p1b25,  # Zeile 3
            global.p1b26, global.p1b27   # Zeile 4
        ]
        ...
        for i : 0 .. 7
            if ds_labels[i] != nil
                if i < size(ds_sensors)
                    ...
                    # Format: ..ABCD 22.5°C
                    ds_labels[i].set_text(string.format("..%s %.1f°C", sensor['id'], sensor['temp']))
                else
                    ds_labels[i].set_text("")
                end
            end
        end
    end
